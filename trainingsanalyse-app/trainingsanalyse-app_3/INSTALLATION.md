...
6. **Wichtig, Korrektur:** Der kostenlose **Free**-Tarif unterstützt **keine
   persistenten Disks** (Render zeigt das als Hinweis direkt im Formular an) – den
   "Disks"-Schritt gibt es dort also gar nicht zum Anklicken, einfach überspringen.
   Das bedeutet: deine Daten (Aktivitäten, Trainingsplan) liegen nur im
   Server-Dateisystem und werden bei einem **Redeploy** (z.B. nach einem
   Code-Update) zurückgesetzt – ein normales Einschlafen bei Inaktivität ist
   dagegen unproblematisch, das übersteht die Daten. Für den Start reicht das
   völlig; deine echten Trainingsdaten in Apple Health sind davon so oder so nicht
   betroffen. Wenn du später eine robustere, weiterhin kostenlose Lösung willst,
   sag Bescheid – das lässt sich nachrüsten (z.B. mit einem kostenlosen externen
   Speicherdienst).
